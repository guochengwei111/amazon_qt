...
class DownEmail(object):
    ...
    def get_att(self, msg_in, str_day=""):
        ...
        for part in msg_in.walk():
            ...
            file_name = part.get_filename()  # 获取file_name的第2种方法
            ...
            if file_name:
                ...
                if not part.is_multipart():
                    ...
                    try:
                        ...
                        print(e)
                        print(part.get_payload(decode=True).decode('gb18030'))
    ...
    def run_ing(self):
        ...
        for i in range(index, 0, -1):  # 倒序遍历邮件
            ...
            # 方法2：from or Form均可
            the_from = parseaddr(msg.get('from'))[1]
            the_to = parseaddr(msg.get('To'))[1]
            the_cc = parseaddr(msg.get_all('Cc'))[1]  # 抄送人
            the_subject = self.decode_str(msg.get('Subject'))
            print('from:%s,to:%s,Cc:%s,subject:%s' % (the_from, the_to, the_cc, the_subject))
            # 获取邮件时间,格式化收件时间
...
if __name__ == '__main__':
    ...
    try:
        # 输入邮件地址, 口令和POP3服务器地址:
        user = '185381664@qq.com'
        # 此处密码是授权码,用于登录第三方邮件客户端
        password = 'xbgioqfzuairbhie'
        eamil_server = 'pop.qq.com'
        email_class = DownEmail(user=user, password=password, eamil_server=eamil_server)
        email_class.run_ing()
    except Exception as e:
        import traceback
